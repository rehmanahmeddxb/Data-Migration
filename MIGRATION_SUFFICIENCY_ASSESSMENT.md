...
No repository file was modified by this session (working tree clean; all scratch output in `/tmp`).
...
### G1 — 🔴 HIGH · NEW: no guard against an *already migrated* OLD file → silent corruption
...
### G2 — 🟠 MEDIUM-HIGH: new-schema columns arrive NULL, and the report never says so
...
### G3 — 🟠 MEDIUM: schema drift fails late and unhelpfully
...
### G4 — 🟠 MEDIUM: `RESULT: PASS` proves counts, not values
...
### G5 — 🟡 MEDIUM (operational): the app can refuse to boot after a migration
...
Before running it on **production**, do these four things (≈ half a day of work):

1. **G1** — refuse/warn when OLD already looks v4.4 (prevents silent audit-trail corruption).
2. **G3** — pre-flight `NOT NULL` column check with an actionable error.
3. **G4** — add value/checksum parity so `PASS` means values, not just counts.
4. **G2 + G5** — report the NULL-filled columns and print the boot-day runbook hint.

And re-run the §2.6 template-vs-models check whenever the app's models change.
